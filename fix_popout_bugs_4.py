...
# 3. Fix createPopoutFromObject Viewport Transform timing
...
match = slider_events_regex.search(content)
if match:
    content = content[:match.start()] + new_bind_inputs + content[match.end():]
else:
    print("Could not find slider events to replace!")
