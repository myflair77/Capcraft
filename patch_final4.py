...
# 1. Replace syncShapeToText
old_sync = re.search(r'function syncShapeToText\(shape, transform\) \{.*?\n        \}', content, re.DOTALL).group(0)
...
# 2. Add isWidthFixed: true to Textbox creation
content = content.replace("splitByGrapheme: true,\n                hoverCursor: 'move'\n            });", "splitByGrapheme: true,\n                isWidthFixed: true,\n                hoverCursor: 'move'\n            });")

# 3. Replace textObj.on('changed')
old_changed = re.search(r'textObj\.on\(\\\'changed\\\', function\(\) \{.*?\n                \}\n            \}\);', content, re.DOTALL).group(0)
...
# 6. Update updateActiveText to handle dirty and initDimensions
old_update_text = re.search(r'function updateActiveText\(propName = null, propValue = null, skipSelection = false\) \{.*?canvas\.requestRenderAll\(\);\n            \}\n        \}', content, re.DOTALL).group(0)
